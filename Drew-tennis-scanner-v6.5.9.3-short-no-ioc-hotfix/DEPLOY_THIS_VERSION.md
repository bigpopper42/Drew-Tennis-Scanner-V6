...
3. Commit and push.
4. Redeploy Railway.
5. Confirm the startup log and Discord connection message report Version `6.5.9.3` and `Polymarket execution: LIVE`.

No Supabase migration and no new Railway environment variable are required.

## Entry-order behavior

Version 6.5.9.3 uses price-capped IOC limit entries for both LONG/YES and SHORT/NO. A SHORT/NO order still sends the explicit NO outcome fields, while its limit price is correctly expressed using Polymarket's required YES-reference price.
...
The proposed emergency exit trigger is now **40¢ on the backed outcome**, but this stop feature is not active in Version 6.5.9.3.
