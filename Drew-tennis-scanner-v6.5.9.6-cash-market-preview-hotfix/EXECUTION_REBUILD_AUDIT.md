# Polymarket Execution Rebuild Audit — Version 6.5.9.6

## Bottom line

Version 6.5.9.6 does **not** reuse the V6.5.6–V6.5.8 execution matcher. The live execution authority is now `scanner/polymarket_executor.py`; `scanner/execution.py` is only a compatibility re-export.
...
After Version 6.5.9.5 was deployed, the live preview endpoint returned `cash_order_qty is required for market order`. Version 6.5.9.6 incorporates that direct production evidence by restoring `cashOrderQty` and adding an exact regression test.
...
| Requirement | Official contract reviewed | V6.5.9.6 implementation |
|---|---|---|
...
| Cash-sized market order | The live preview endpoint requires `cashOrderQty` for market orders and supports intent or `outcomeSide` + `action`, IOC, automatic indicator, and `slippageTolerance` | `order_request` sends both direction forms, exact 20% USD `cashOrderQty`, and a three-tick adverse-price cap |
...
| Real-time state | Official docs recommend private WebSocket updates over polling | V6.5.9.6 uses bounded REST polling; WebSocket remains a documented limitation |
...
- Full automated suite: **162 tests passed** in the source tree.
- Replacement archive: extracted into a clean directory and the same **162 tests passed** again.
- Full Python compilation: passed in the source tree and clean extraction.
- Dry-run Railway configuration startup: passed with Version `6.5.9.6`, cash-sized 20% sizing, unlimited distinct markets, and same-market upgrades disabled.
...
V6.5.9.6 is materially different from the failed V6.5.6–V6.5.8 matcher and is suitable for a controlled Railway integration test. It is **not** described as guaranteed or live-proven.
