# Drew Tennis Scanner Version 6.5.9.5

Version 6.5.9.5 leaves Drew's tennis decision rules unchanged and replaces the live Polymarket execution path with a separate event-first executor.
...
- Uses a quantity-based market order with a three-tick slippage cap, sized at the worst allowed price so maximum cost stays at or below exactly 20% of authenticated account balance, limited by buying power.
...
- Quantity-based market order with bounded slippage and automatic-order regulatory indicator.
...
4. Confirm startup reports Version `6.5.9.5` and `Polymarket execution: LIVE`.
