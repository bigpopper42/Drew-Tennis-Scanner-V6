...
class PolymarketExecutionEngine:
    ...
    def execute_trade(self, record: Mapping[str, Any]) -> ExecutionResult:
        ...
        try:
            ...
            configured_slippage_ticks = max(0, int(self.config.slippage_ticks))
            available_upside_ticks = max(
                0,
                int(
                    ((Decimal("0.99") - player_price) / tick_size).to_integral_value(
                        rounding=ROUND_DOWN
                    )
                ),
            )
            slippage_ticks = min(configured_slippage_ticks, available_upside_ticks)
            adverse_player_price = player_price + tick_size * Decimal(slippage_ticks)
            ...
            order_request = {
                ...
                "maxBlockTime": "5",
                "slippageTolerance": {
                    # Unlike limit-order price.value, slippageTolerance.currentPrice
                    # is the current price of the outcome being purchased. For a
                    # BUY_SHORT/NO order, sending the inverted YES reference here
                    # makes the exchange protect the wrong side of the price scale
                    # and can expire an otherwise executable IOC order.
                    "currentPrice": {
                        "value": _decimal_string(player_price),
                        "currency": "USD",
                    },
